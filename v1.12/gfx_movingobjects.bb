...
Function mobj_addtostack(Mobj.MObj)
	...
	DebugLog "mobj_insertstack i>"+Mobj\depth
...
Function MObjSetShadowImageSequence(t.MObj,sequence.imagesequence,xoff,yoff)
...
Function MObjCollide(o.MObj,p.MObj)

	img1=o\gfxsequence\image[imageix]
	img2=p\gfxsequence\image[imageix]
	x1=o\x
	y1=o\y
	x2=p\x
	y2=p\y
	
	if(ImagesOverlap (img1,x1,y1,img2,x2,y2)) then
	
		return ImagesCollide (img1,x1,y1,0,img2,x2,y2,0)
...
Function MObjCollideCustom(o.MObj,img2,x2,y2)
...
Function ShowMObjs(offsetx,offsety,buffer)
	...
	For t=0 To mobj_stacksize
		If t >=mobj_stackcounter Then Exit 
			...
			If(s\shadow<>Null) Then 
			
				DrawImage(s\shadow\image[image],s\x+s\shadowx+offsetx,s\y+s\shadowy+offsety)
...
Function CountActiveMObjs()

	return mobj_stackcounter
...
Function CountActiveMObjsOfType(id)

			
	Count=0	
	For s.MObj = Each MObj
		 
		If s\visible=True And s\id=id And s\hidden=False
			Count=Count+1
